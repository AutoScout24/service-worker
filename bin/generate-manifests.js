...
urls.forEach(url => {
    axios.get(url).then(res => {
        ...
        fs.writeFileSync(`manifest.${tld}.json`, JSON.stringify(manifest, null, 4));
    });
});

fs.writeFileSync(`manifest.localhost.json`, JSON.stringify(createManifest('Test description'), null, 4));
